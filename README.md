...
graph LR
    ...
    Manifests -- "ArgoCD sync" --> K8s
```

| Repository | Contains | Who owns it |
|---|---|---|
| `my-api`, `my-frontend` | Application source code, Dockerfile, tests, CI pipeline | Development teams |
| `gitops-repo` (generated by gostrap) | Kubernetes manifests, environment overlays, ArgoCD config | Platform / DevOps team |
| `gostrap` (this repo) | The CLI tool itself | Open source |

The deployment flow is: developers push code → CI builds and pushes a container image → the image tag is updated in the gitops-repo (via PR or automation) → ArgoCD detects the change and syncs to the cluster.
